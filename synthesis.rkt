...
(define (??expr terminals depth)
  ...
  (define a (if (= depth 0) (apply choose* terminals)  (apply choose* (cons (??expr terminals (sub1 depth)) terminals))))
  (define b (if (= depth 0) (apply choose* terminals)  (apply choose* (cons (??expr terminals (sub1 depth)) terminals))))
...
(define prog
  (interpret (element* '((2 1) (1 2)) x)))
;  (interpret (eye-like x)))

(define M
  (synthesize
    #:forall (list x)
    #:guarantee (assert (equal? (interpret sketch) prog))))
;    #:guarantee (assert (equal? '((1 0) (0 1)) prog))))
