...
class Components:
    ...
    def matmul(self, shape1, shape2):
        assert shape1[1] == shape2[0]
        I = [[[z3.Int(self.get_name()) for j in range(shape1[1])] for i in range(shape1[0])],
             [[z3.Int(self.get_name()) for j in range(shape2[1])] for i in range(shape2[0])]]
        O = [[z3.Int(self.get_name()) for j in range(shape2[1])] for i in range(shape1[0])]
        phi = []
